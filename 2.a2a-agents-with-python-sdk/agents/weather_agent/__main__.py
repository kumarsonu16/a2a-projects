...
Now we need to wrap our intelligent agent in a proper web server that speaks the A2A protocol. 
This server will handle HTTP requests, manage agent discovery, and coordinate the entire system.
...
def main(host: str, port: int):
    ...
    """
    The server uses Starlette (a modern ASGI framework) wrapped in A2A’s application layer. 
    This gives us high-performance async handling while maintaining A2A protocol compliance.

    The DefaultRequestHandler connects all the pieces:
        1. Our WeatherAgentExecutor handles the actual weather logic
        2. InMemoryTaskStore manages task state
        3. BasePushNotificationSender handles real-time updates to clients
    """
